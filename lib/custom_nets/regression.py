...
from .regression_unet_blocks import *
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim = (num_channels[0] * 4) if num_class_embeds else 0
    ...
    def forward(
        ...
        class_labels: torch.Tensor | int | None = None,
        ...
        """
        # 1. time
        emb = torch.zeros((x.shape[0], 0), device=x.device)  # dummmy
        ...
        if self.num_class_embeds is not None:
            ...
            if isinstance(class_labels, int):
                class_labels = torch.full(
                    (x.shape[0],), class_labels, device=x.device, dtype=torch.long
                )
